...
def predict_text(text):
    ...
    dic_ec  = rtc.readTxt("models/data/dic_ecc.txt")
    print(dic_epc)
    print(dic_mbm)
    print(dic_ec)
    dic_epc_SBP = stemmerBolsaPalabras(dic_epc)
    dic_mbm_SBP = stemmerBolsaPalabras(dic_mbm)
    dic_ec_SBP  = stemmerBolsaPalabras(dic_ec)
    print(dic_epc_SBP)
    print(dic_mbm_SBP)
    print(dic_ec_SBP)

    jacc_epc_mbm_ec = jcc.jaccard(txt_nlp[0],[dic_epc_SBP,dic_mbm_SBP,dic_ec_SBP])

    print(jacc_epc_mbm_ec)
    cos_epc_mbm_ec   = np.array([csc.coseno(txt_nlp,dic_epc_SBP) ,csc.coseno(txt_nlp,dic_mbm_SBP) ,csc.coseno(txt_nlp,dic_ec_SBP)])
